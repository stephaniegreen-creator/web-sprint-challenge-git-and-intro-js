...
function removeArtist(artists, number) {
  let copyOfArray = [...artists];
  copyOfArray.splice(number,1);
...
function artistInfo(artists, name){
  let artInfo = artists.filter(x => x.name === name);
  return artInfo[0].bio
}
...
function artistByCountry(arr, string){
  ...
  for(let i = 0; i < arr.length; i++) {
    if(arr[i].nationality === string) {
      nationLy.push(arr[i].name);
    }
  } return nationLy;
   
}

console.log(artistByCountry(artists, "Mexican"));
